...
public class QuestCaptureFugitives extends QuestBase implements Quest
{
	...
	private boolean perform( DataWrapper quest )
	{
		...
		if ( !quest.getData().getCompleted() )
		{
			...
			quest.getData().getPlayer().sendStatusMessage(new TextComponentString("Captured " + MathHelper.clamp(quest.getCurrentAmount(), 0, quest.getTargetAmount()) + "/" + quest.getTargetAmount() + " Fugitives"), true);
			// quest.getData().getPlayer().sendStatusMessage( new
			// TextComponentString("Captured " + MathHelper.clamp(quest.getCurrentAmount(),
			// 0, quest.getTargetAmount())+" out of "+quest.getTargetAmount() + "
			// fugitives"), true);
	...
	public List<ItemStack> accept( QuestData data, List<ItemStack> in )
	{
		...
		// //addToroSpawner(data, data.getPlayer().getEntityWorld(), pos,
		// getDefaultEnemies(data));
		// }
		// catch (Exception e)
		// {
		//
		...
		try
		{
			...
			while (tries > 0)
			{
				tries--;
				{
					...
					BlockPos loc = new BlockPos(x, EventHandlers.MAX_SPAWN_HEIGHT, z);
	...
	// private void addToroSpawner(QuestData data, World world, BlockPos blockpos,
