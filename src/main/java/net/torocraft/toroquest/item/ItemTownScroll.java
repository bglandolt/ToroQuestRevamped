...
public class ItemTownScroll extends Item
{
	...
	public static void registerRenders()
	{
		 ...
		 * + NAME + civ.getCivName(), "inventory");
		 * Minecraft.getMinecraft().getRenderItem().getItemModelMesher().register(
		 * INSTANCE, i, model);
		 * RenderItem renderItem = Minecraft.getMinecraft().getRenderItem();
		 * renderItem.getItemModelMesher().register(INSTANCE, i, new
		 * ModelResourceLocation(ToroQuest.MODID + ":" + NAME + civ.getCivName(),
		 * "inventory"));
		 * i++;
		 * }
		 */
	...
	public ItemStack onItemUseFinish( ItemStack stack, World worldIn, EntityLivingBase entityLiving )
	{
		...
		try
		{
			...
			BlockPos teleportLocation = EventHandlers.findTeleportLocationFrom(entityplayer.world, loc);
			...
			if ( entityplayer != null && !entityplayer.capabilities.isCreativeMode ) // && province.civilization != null
																					 // &&
																					 // PlayerCivilizationCapabilityImpl.get(entityplayer).getReputation(province.civilization)
																					 // < 2000 )
			{
				stack.shrink(1);
			}
			...
			// entityplayer.world.playSound((EntityPlayer)null, entityplayer.posX,
			// entityplayer.posY, entityplayer.posZ, SoundEvents.BLOCK_PORTAL_TRAVEL,
	...
	// instance, written books always return
	// * true).
	// *
	// * Note that if you override this method, you generally want to also call the
	// super version (on {@link Item}) to get
	// * the glint for enchanted items. Of course, that is unnecessary if the
	// overwritten version always returns true.
	// */
	// @SideOnly(Side.CLIENT)
