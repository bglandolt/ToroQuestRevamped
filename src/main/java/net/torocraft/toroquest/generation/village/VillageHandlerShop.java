...
{
	protected static final String NAME = "tqr_shop";
	...
	public static class VillagePieceShop extends VillagePieceBlockMap
	{
		...
		{
			if ( ToroQuestConfiguration.disableShop )
				return null;
			...
			return canVillageGoDeeper(bounds) && StructureComponent.findIntersecting(structures, bounds) == null ? new VillagePieceShop(nameType, start, type, rand, bounds, facing) : null;
		}
		...
		protected boolean specialBlockHandling( World world, String c, int x, int y, int z )
		{
			...
			else if ( c.equals("cv") )
			{
				...
				// Blocks.CHEST.getDefaultState().withProperty(BlockChest.FACING,
				// EnumFacing.WEST));
				return true;
			}
			else if ( c.equals("c^") )
		...
		protected void setChestBlockState( World worldIn, IBlockState blockstateIn, int x, int y, int z, StructureBoundingBox boundingboxIn )
		{
			...
			if ( boundingboxIn.isVecInside(blockpos) && !(worldIn.getBlockState(blockpos) instanceof BlockChest) )
			{
				...
				if ( tileentity instanceof TileEntityChest )
				{
					...
					{
						// ((TileEntityChest)
						// tileentity).setLootTable(LootTableList.CHESTS_SIMPLE_DUNGEON,
						...
						// case EARTH:
						// {
						// ItemScrollEarth scroll =
						// (ItemScrollEarth)Item.getByNameOrId("toroquest:scroll_earth");
						...
						// itemstack.setTagInfo("province_name", new
						// NBTTagString(province.name.toString()));
						...
						// case FIRE:
						// {
						// ItemScrollFire scroll =
						// (ItemScrollFire)Item.getByNameOrId("toroquest:scroll_fire");
						// ItemStack itemstack = new ItemStack(scroll,worldIn.rand.nextInt(3)+1);
						// itemstack.setTagInfo("province", new NBTTagString(province.id.toString()));
						// itemstack.setTagInfo("province_name", new
						// NBTTagString(province.name.toString()));
						...
						// case SUN:
						// {
						// ItemScrollSun scroll =
						// (ItemScrollSun)Item.getByNameOrId("toroquest:scroll_sun");
						// ItemStack itemstack = new ItemStack(scroll,worldIn.rand.nextInt(3)+1);
						// itemstack.setTagInfo("province", new NBTTagString(province.id.toString()));
						// itemstack.setTagInfo("province_name", new
						// NBTTagString(province.name.toString()));
						...
						// case WATER:
						// {
						// ItemScrollWater scroll =
						// (ItemScrollWater)Item.getByNameOrId("toroquest:scroll_water");
						// ItemStack itemstack = new ItemStack(scroll,worldIn.rand.nextInt(3)+1);
						// itemstack.setTagInfo("province", new NBTTagString(province.id.toString()));
						// itemstack.setTagInfo("province_name", new
						// NBTTagString(province.name.toString()));
						...
						// case MOON:
						// {
						// ItemScrollMoon scroll =
						// (ItemScrollMoon)Item.getByNameOrId("toroquest:scroll_moon");
						// ItemStack itemstack = new ItemStack(scroll,worldIn.rand.nextInt(3)+1);
						// itemstack.setTagInfo("province", new NBTTagString(province.id.toString()));
						// itemstack.setTagInfo("province_name", new
						// NBTTagString(province.name.toString()));
						...
						// case WIND:
						// {
						// ItemScrollWind scroll =
						// (ItemScrollWind)Item.getByNameOrId("toroquest:scroll_wind");
						// ItemStack itemstack = new ItemStack(scroll,worldIn.rand.nextInt(3)+1);
						// itemstack.setTagInfo("province", new NBTTagString(province.id.toString()));
						// itemstack.setTagInfo("province_name", new
						// NBTTagString(province.name.toString()));
		...
		//
		// //((TileEntityChest)
		// tileentity).setLootTable(LootTableList.CHESTS_STRONGHOLD_LIBRARY,
		// world.rand.nextLong());
		// // ((TileEntityChest)
		...
		// {
		// ((TileEntityChest)
		// tileentity1).setLootTable(LootTableList.CHESTS_SIMPLE_DUNGEON,
		// world.rand.nextLong());
		// }
		// for (int i = 0; i < 17; i++)
		// world.setBlockState(new BlockPos( x, y+8+i, z ),
		// Blocks.STONE.getDefaultState(), i);
		//
		// }
