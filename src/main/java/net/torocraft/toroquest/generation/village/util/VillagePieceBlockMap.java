...
public abstract class VillagePieceBlockMap extends StructureVillagePieces.Village
{
	...
	static
	{
		...
		IBlockState CH = Blocks.IRON_BARS.getDefaultState();
		...
		catch (Exception e)
		{
			try
			{
				CH = Block.getBlockFromName("futuremc:chain").getDefaultState();
			}
			catch (Exception ee)
			{
		...
		try
		{
			CHICKEN = Block.getBlockFromName(ToroQuestConfiguration.turkeyBlockResourceName).getDefaultState();
		}
		catch (Exception e)
		{
		}
		...
		try
		{
			CAKE = Block.getBlockFromName(ToroQuestConfiguration.cakeBlockResourceName).getDefaultState();
		}
		catch (Exception e)
		{
		}
		...
		try
		{
			TABLE = Block.getBlockFromName(ToroQuestConfiguration.tableResourceName).getDefaultState();
		}
		catch (Exception e)
		{
		}
		...
		// COLORED BEDS = $v
		// DEFAULT_PALLETTE.put("aa", Block.getBlockFromItem( new ItemStack(Items.BED,
		// 1, 3).getItem() ).getDefaultState());
		...
		try
		{
			ET = Block.getBlockFromName(ToroQuestConfiguration.enchantingTableResourceName).getDefaultState();
		}
		catch (Exception e)
		{
		}
		...
		// DEFAULT_PALLETTE.put("FR",
		// Blocks.FLOWER_POT.getDefaultState().withProperty(BlockFlowerPot.CONTENTS,
		// EnumFlowerType.POPPY));
		// DEFAULT_PALLETTE.put("FU",
		// Blocks.FLOWER_POT.getDefaultState().withProperty(BlockFlowerPot.CONTENTS,
		// EnumFlowerType.BLUE_ORCHID));
		// DEFAULT_PALLETTE.put("FG",
		// Blocks.FLOWER_POT.getDefaultState().withProperty(BlockFlowerPot.CONTENTS,
		// EnumFlowerType.OAK_SAPLING));
		// DEFAULT_PALLETTE.put("FB",
		// Blocks.FLOWER_POT.getDefaultState().withProperty(BlockFlowerPot.CONTENTS,
		// EnumFlowerType.MUSHROOM_BROWN));
		// DEFAULT_PALLETTE.put("FY",
		// Blocks.FLOWER_POT.getDefaultState().withProperty(BlockFlowerPot.CONTENTS,
		// EnumFlowerType.CACTUS));
		// DEFAULT_PALLETTE.put("FW",
		// Blocks.FLOWER_POT.getDefaultState().withProperty(BlockFlowerPot.CONTENTS,
		// EnumFlowerType.FERN));
	...
	// // helmet
	// protected boolean specialHandlingForSpawnerH(World world, String
	// entityBlockCode, String c, int x, int y, int z, List<String> entities)
	// {
	...
	// }
	//
	// BlockPos blockpos = new BlockPos(this.getXWithOffset(x, z),
	...
	// return true;
	// }
	// protected void addToroSpawnerH(World world, BlockPos blockpos, List<String>
	// entities)
	// {
	// try
	// {
	...
	// ((TileEntityToroSpawner) tileentity).setHelmet(new
	// ItemStack(Item.getByNameOrId("toroquest:bandit_helmet")));
	// }
	// }
	// catch(Exception e)
	// {
	...
	public boolean addComponentParts( final World world, Random randomIn, StructureBoundingBox structureBoundingBoxIn )
	{
		...
		new BlockMapBuilder(name)
		{
			...
			/**
			 * Flag 1 will cause a block update. Flag 2 will send the change to clients.
			 * Flag 4 will prevent the block from
			 * being re-rendered, if this is a client world. Flag 8 will force any
			 * re-renders to run on the main thread instead
			 * of the worker pool, if this is a client world and flag 4 is clear. Flag 16
			 * will prevent observers from seeing
			...
			// try
			// {
			// CivilizationType civ = CivilizationUtil.getProvinceAt(world,
			// ((structureBoundingBoxIn.minX+structureBoundingBoxIn.maxX)/32),
			// ((structureBoundingBoxIn.minZ+structureBoundingBoxIn.maxZ)/32)).civilization;
			// //System.out.println(civ);
			//
	...
	// try
	// {
	// CivilizationType civ = CivilizationUtil.getProvinceAt(world, pos.getX()/16,
	// pos.getZ()/16).civilization;
	// switch ( civ )
	// {
	// case FIRE:
	// {
	...
	// public static boolean setb(World worldIn, @Nullable EntityPlayer player,
	// BlockPos pos, ItemStack stackIn)
	// {
	// {
