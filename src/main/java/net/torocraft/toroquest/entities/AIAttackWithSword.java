...
public class AIAttackWithSword extends EntityAIBase
{
	...
	protected void checkAndPerformAttack( EntityLivingBase victim, double distanceSq )
	{
		...
		if ( !this.attacker.collidedHorizontally && !backPeddaling && !this.attacker.isHandActive() ) // this.attacker.getNavigator().getPathToEntityLiving(victim)
																										 // != null
		{
		...
		else if ( distanceSq <= attackDistance )
		{
			if ( this.attackTick <= 0 )
			{
				if ( !offhandAttack )
				{
					...
					this.attackTick = this.bigWeapon ? 30 + rand.nextInt(11) : 20 + rand.nextInt(11);
					this.attacker.attackEntityAsMob(victim);
				}
				else
				{
					this.attacker.swingArm(EnumHand.OFF_HAND);
					this.attackTick = this.bigWeapon ? 18 + rand.nextInt(9) : 12 + rand.nextInt(9);
					offhandAttack = false;
					this.attacker.attackEntityAsMob(victim);
					return;
				}
