...
public class EntityAIToroVillagerMate extends EntityAIBase
{
		...
		if ( this.checkSufficientDoorsPresentForNewVillager(rep) && this.villager.getIsWillingToMate(true) )
		{
			...
			List<EntityToroVillager> entities = this.world.getEntitiesWithinAABB(EntityToroVillager.class, new AxisAlignedBB(this.villager.getPosition()).grow(10, 6, 10), new Predicate<EntityToroVillager>()
			{
				public boolean apply( @Nullable EntityToroVillager entity )
				{
					...
					// if ( entity != villager && entity.getGrowingAge() == 0 && !entity.isChild()
					// && entity.underAttack == null && entity.blockedTrade < 1 )
					// {
					// return true;
					// }
					// return false;
			...
			for ( EntityToroVillager entity : entities )
			{
				if ( entity != villager && entity.getGrowingAge() == 0 && !entity.isChild() && entity.underAttack == null && entity.blockedTrade < 1 && entity.isEntityAlive() )
				{
					...
					if ( this.mate.getNavigator().tryMoveToXYZ(this.villager.bedLocation.getX(), this.villager.bedLocation.getY(), this.villager.bedLocation.getZ(), 0.5D) && this.villager.getNavigator().tryMoveToXYZ(this.villager.bedLocation.getX(), this.villager.bedLocation.getY(), this.villager.bedLocation.getZ(), 0.5D) )
					{
						return true;
					}
				}
	...
	public void updateTask()
	{
		if ( this.villager != null && this.villager.bedLocation != null && this.mate != null && this.mate.bedLocation != null )
		{
			...
			if // both on the bed
			( (this.villager.getDistance(this.villager.bedLocation.getX(), this.villager.bedLocation.getY(), this.villager.bedLocation.getZ()) < 1.75 || this.villager.getNavigator().tryMoveToXYZ(this.villager.bedLocation.getX(), this.villager.bedLocation.getY(), this.villager.bedLocation.getZ(), 0.5D))

				&& (this.mate.getDistance(this.villager.bedLocation.getX(), this.villager.bedLocation.getY(), this.villager.bedLocation.getZ()) < 1.75 || this.mate.getNavigator().tryMoveToXYZ(this.villager.bedLocation.getX(), this.villager.bedLocation.getY(), this.villager.bedLocation.getZ(), 0.5D)) )
			{
