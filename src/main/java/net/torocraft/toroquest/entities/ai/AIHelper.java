...
public interface AIHelper
{
	...
	// if (e.world instanceof WorldServer)
	// {
	// ((WorldServer)e.world).spawnParticle(EnumParticleTypes.SWEEP_ATTACK,
	// target.posX + d0, target.posY + (double)target.height * 0.5D, target.posZ +
	// d1, 0, d0, 0.0D, d1, 0.0D);
	// }
	// }
	...
	// double d0 = (p.getPositionVector().x - in.getPositionVector().x) * 2;
	// double d2 = (p.getPositionVector().z - in.getPositionVector().z) * 2;
	// float f = (float)(MathHelper.atan2(d2+prevRotationYaw*turnSpeed, d0) * (180D
	// / Math.PI)) - 90.0F;
	// in.rotationYaw = f;
	// in.prevRotationYaw = f;
	// }
	...
	// PathPoint p2 =
	// in.getNavigator().getPath().getPathPointFromIndex(in.getNavigator().getPath().getCurrentPathIndex());
	//
