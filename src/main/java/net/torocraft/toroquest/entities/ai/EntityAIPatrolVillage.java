...
public class EntityAIPatrolVillage extends EntityAIBase
{
	...
	public EntityAIPatrolVillage( EntityGuard entityIn, double movementSpeedIn )
	{
		...
		// {
		// throw new IllegalArgumentException("Unsupported mob for
	...
	{
		// if (this.entity.getNavigator().noPath() ||
		// this.entity.getDistanceSq(this.doorInfo.getDoorBlockPos()) < 16.0D)
		// {
	...
	// {
	// ArrayList<VillageDoorInfo> villagedoorinfo = new
	// ArrayList<VillageDoorInfo>();
	//
	// int i = Integer.MAX_VALUE;
	//
	// for ( VillageDoorInfo v : villageIn.getVillageDoorInfoList() )
	// {
	// int j = v.getDistanceSquared(MathHelper.floor(this.entity.posX),
	// MathHelper.floor(this.entity.posY), MathHelper.floor(this.entity.posZ));
	//
