...
public class EntitySentry extends EntityToroMob implements IRangedAttackMob, IMob
{
	...
	{
		EntityRegistry.registerModEntity(new ResourceLocation(ToroQuest.MODID, NAME), EntitySentry.class, NAME, entityId, ToroQuest.INSTANCE, 80, 1, true, 0x8f3026, 0xe0d359);
	}
	...
	// private static final AttributeModifier SPRINTING_SPEED_BOOST = (new
	// AttributeModifier(SPRINTING_SPEED_BOOST_ID, "Sprinting speed boost",
	// 10.30000001192092896D, 2)).setSaved(false);

	@Override
	public void setSprinting( boolean b )
	{
		if ( this.getAttackTarget() != null && !this.isFleeing() && !(this.getHeldItemMainhand().getItem() instanceof ItemBow) )
	...
	public boolean processInteract( EntityPlayer player, EnumHand hand )
	{
		...
		if ( this.getHealth() >= this.getMaxHealth() && !this.inCombat() )
		{
			if ( !this.getBribed() )
			{
				/* BRIBE WITH EMERALDS */
				if ( itemstack.getItem() == Items.EMERALD )
				{
					...
					List<EntitySentry> bandits = this.world.<EntitySentry>getEntitiesWithinAABB(EntitySentry.class, new AxisAlignedBB(this.getPosition()).grow(25, 12, 25));
					{
						for ( EntitySentry bandit : bandits )
						{
							if ( !bandit.getBribed() && bandit.passiveTimer < 4 )
							{
				...
				{
					if ( rand.nextBoolean() || this.passiveTimer < 1 )
					{
			...
			{
				/* RECRUIT WITH PAPERS, BRIBED */
				if ( ToroQuestConfiguration.recruitBandits && item.equals(Item.getByNameOrId("toroquest:recruitment_papers")) )
				{
					this.playSound(SoundEvents.ENTITY_ILLAGER_CAST_SPELL, 1.2F, 1.2F);
					this.playSound(SoundEvents.ITEM_ARMOR_EQUIP_CHAIN, 1.0F, 1.0F);
					this.playSound(SoundEvents.BLOCK_ANVIL_USE, 0.5F, 0.8F);

					itemstack.shrink(1);

					EntityGuard newEntity = new EntityGuard(this.world);
					...
					// List<EntitySentry> bandits =
					// this.world.getEntitiesWithinAABB(EntitySentry.class, new
					// AxisAlignedBB(this.getPosition()).grow(32, 16, 32), new
					// Predicate<EntitySentry>()
					// {
	...
	public void dropLoot()
	{
		if ( !this.world.isRemote )
		{
			...
			{
				// if ( this.getHeldItemOffhand().getItem() instanceof ItemPotion &&
				// rand.nextBoolean() )
				// {
				// ItemStack stack = new ItemStack(this.getHeldItemOffhand().getItem(), 1);
				// EntityItem dropItem = new EntityItem(this.world, this.posX, this.posY,
				// this.posZ, stack.copy());
				// this.world.spawnEntity(dropItem);
	...
	protected void initEntityAI()
	{
		...
		if ( !(this instanceof EntityOrc) )
		{
			this.tasks.addTask(7, new EntityAISmartTempt(this, 0.55D, Items.EMERALD)
			{
				...
				// {
				// super.shouldExecute();
				// if ( EntitySentry.this.getAttackTarget() != null ||
				// EntitySentry.this.isBurning() )
				// {
				// return false;
				// }
				// return super.shouldExecute();
	...
	public void onLivingUpdate()
	{
		...
		if ( !this.world.isRemote )
		{
			if ( this.getRidingEntity() instanceof EntityBoat )
			{
				if ( this.getRidingEntity() == this.boat )
				{
					...
					// if ( this.boat.collidedHorizontally || this.boat.collidedVertically )
					// this.boat.setDead(); // ( this.boat.isEntityInsideOpaqueBlock() )
					// this.boat.setDead();
		...
		if ( this.flanking )
		{
			if ( this.getNavigator().noPath() || this.getNavigator().getPath().getFinalPathPoint() == null || this.getAttackTarget() == null || this.getDistance(this.getAttackTarget()) >= 7.5 || this.blocking || this.isFleeing() )
			{
				this.flanking = false;
		...
		if ( this.ticksExisted % 100 == 0 )
		{
			...
			if ( this.getAttackTarget() != null )
			{
				...
				// ==========================================================================================
				// ==========================================================================================
				if ( !this.isDrinkingPotion() && !this.blocking && !this.isFleeing() && !(this.getHeldItemMainhand().getItem() instanceof ItemBow) && this.getDistance(this.getAttackTarget()) <= 6.5 && !this.isInWater() ) // &&
																																																							 // this.rand.nextBoolean()
																																																							 // // &&
																																																							 // this.getDistance(this.getAttackTarget())
																																																							 // > 2 )
				{
					try
					{
				...
				else if ( this.aggroTimer++ >= 5 && this.climbingTimer <= 0 && !this.canEntityBeSeen(this.getAttackTarget()) ) // &&
																																 // this.getDistance(this.getAttackTarget())
																																 // > 12 &&
																																 // !(this.getHeldItemMainhand().getItem()
																																 // instanceof ItemBow) )
				{
	...
	public boolean attackEntityFrom( DamageSource source, float amount )
	{
					...
					else
					{
						...
						if ( dist < 16 )
						{
							if ( this.canShieldPush )
							{
								...
								this.knockBackSmart(e, (float) (0.3D - dist / 100.0D));
		...
		{
			if ( !this.canEntityBeSeen(source.getTrueSource()) || (this.getDistance(source.getTrueSource()) >= 8.0D && this.getNavigator().getPathToEntityLiving(source.getTrueSource()) == null) )
			{
				Vec3d vec3d = RandomPositionGenerator.findRandomTargetBlockAwayFrom(this, 16, 6, source.getTrueSource().getPositionVector());
	...
	private void sentryTypeDPS()
	{
		if ( this.getAttackTarget() != null && this.getAttackTarget().isEntityAlive() && this.getAttackTarget().getClass() != this.getClass() )
		{
			...
			if ( !this.isFleeing() && !this.flanking )
			{
				...
				// else if ( this.getAttackTarget().onGround &&
				// !this.getAttackTarget().isAirBorne )
				// {
				// this.getMoveHelper().strafe( 0.0F, 0.0F );
				//
				// if ( !( (this.getAttackTarget().posY-this.posY)*2.0D >
				// this.getDistance(this.getAttackTarget()) ) &&
				// !this.collidedHorizontallyWide(this.posY+this.height) )
				// {
	...
	private void sentryTypeRanged() // zzz
	{
		...
		// }
		// else if (
		// this.getEntityAttribute(SharedMonsterAttributes.MOVEMENT_SPEED).getBaseValue()
		// <= 0.0D )
		// {
	...
	// try
	// {
	// IBlockState block = this.world.getBlockState(new
	// BlockPos(((int)this.posX)+1.5D,d,((int)this.posZ)+0.5D));
	// if ( block.isFullCube() )
	// {
	// return true;
	// }
	// block = this.world.getBlockState(new
	// BlockPos(((int)this.posX)+0.5D,d,((int)this.posZ)+1.5D));
	// if ( block.isFullCube() )
	// {
	// return true;
	// }
	// block = this.world.getBlockState(new
	// BlockPos(((int)this.posX)-0.5D,d,((int)this.posZ)+0.5D));
	// if ( block.isFullCube() )
	// {
	// return true;
	// }
	...
	{
		if ( this.climbingTimer < 250 && !(this.getHeldItemMainhand().getItem() instanceof ItemBow) && (this.getAttackTarget().posY - 1.0D > this.posY || this.climbUntilPlatform) ) // &&
	...
	public void addEquipment()
	{
		...
		if ( weapon == 0 ) // MELEE
		{
			if ( rand.nextBoolean() ) // TWO-HANDED
			{
				...
				if ( this.rand.nextInt((int) this.getMaxHealth()) > ToroQuestConfiguration.banditBaseHealth )
					this.weaponMain = new ItemStack(Item.getByNameOrId(ToroQuestConfiguration.banditTwoHandedMeleeWeaponsPowerful[rand.nextInt(ToroQuestConfiguration.banditTwoHandedMeleeWeaponsPowerful.length)]), 1);
			...
			else // DUAL-WIELD
			{
				...
				if ( this.rand.nextInt((int) this.getMaxHealth()) > ToroQuestConfiguration.banditBaseHealth )
					this.weaponMain = new ItemStack(Item.getByNameOrId(ToroQuestConfiguration.banditOneHandedMeleeWeaponsPowerful[rand.nextInt(ToroQuestConfiguration.banditOneHandedMeleeWeaponsPowerful.length)]), 1);
		...
		else if ( weapon == 1 ) // SHIELD
		{
			...
			if ( this.rand.nextInt((int) this.getMaxHealth()) > ToroQuestConfiguration.banditBaseHealth )
				this.weaponOff = new ItemStack(Item.getByNameOrId(ToroQuestConfiguration.banditShieldsPowerful[rand.nextInt(ToroQuestConfiguration.banditShieldsPowerful.length)]), 1);
		...
		else if ( weapon == 2 ) // RANGED
		{
			this.weaponMain = new ItemStack(Item.getByNameOrId(ToroQuestConfiguration.banditRangedWeapons[rand.nextInt(ToroQuestConfiguration.banditRangedWeapons.length)]), 1);
			if ( this.rand.nextInt((int) this.getMaxHealth()) > ToroQuestConfiguration.banditBaseHealth )
				this.weaponMain = new ItemStack(Item.getByNameOrId(ToroQuestConfiguration.banditRangedWeaponsPowerful[rand.nextInt(ToroQuestConfiguration.banditRangedWeaponsPowerful.length)]), 1);
