...
public class EntityBas extends EntitySkeleton implements IMob
{
	...
	{
		EntityRegistry.registerModEntity(new ResourceLocation(ToroQuest.MODID, NAME), EntityBas.class, NAME, entityId, ToroQuest.INSTANCE, 80, 3, true, 0xffffff, 0x909090);
	}
	...
	protected void setEquipmentBasedOnDifficulty( DifficultyInstance difficulty )
	{
		...
		// setItemStackToSlot(EntityEquipmentSlot.MAINHAND, new
		// ItemStack(Items.STONE_SWORD));
		// setItemStackToSlot(EntityEquipmentSlot.HEAD, colorArmor(new
		// ItemStack(Items.LEATHER_HELMET, 1), 0xb0b0b0));
	}
	...
	public void spawnSweepParticles()
	{
		...
		if ( this.world instanceof WorldServer )
		{
			for ( int i = 16; i > 0; i-- )
			{
				...
				// ((WorldServer) this.world).spawnParticle(EnumParticleTypes.SPELL_INSTANT,
				// xx+this.rand.nextGaussian()/10.0D, yy+this.rand.nextGaussian()/10.0D,
				// zz+this.rand.nextGaussian()/10.0D, 0, 0, 0, 0, 0.01D, new int[0]);
				// ((WorldServer) this.world).spawnParticle(EnumParticleTypes.SPELL,
				// xx+this.rand.nextGaussian()/10.0D, yy+this.rand.nextGaussian()/10.0D,
				// zz+this.rand.nextGaussian()/10.0D, 0, 0, 0, 0, 0.01D, new int[0]);
				// ((WorldServer) this.world).spawnParticle(EnumParticleTypes.SPELL_MOB,
				// xx+this.rand.nextGaussian()/10.0D, yy+this.rand.nextGaussian()/10.0D,
				// zz+this.rand.nextGaussian()/10.0D, 0, 0, 0, 0, 0.01D, new int[0]);
				// ((WorldServer) this.world).spawnParticle(EnumParticleTypes.SPELL_MOB_AMBIENT,
				// xx+this.rand.nextGaussian()/10.0D, yy+this.rand.nextGaussian()/10.0D,
				// zz+this.rand.nextGaussian()/10.0D, 0, 0, 0, 0, 0.01D, new int[0]);
	...
	{
		// net.minecraftforge.event.entity.living.EnderTeleportEvent event = new
		// net.minecraftforge.event.entity.living.EnderTeleportEvent(this, x, y, z, 0);
