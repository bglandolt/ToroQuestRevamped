...
public class EntityToroVillager extends EntityVillager implements INpc, IMerchant
{
	...
	{
		if ( this.buyingList == null || this.buyingList.isEmpty() || this.needsInitilization ) // || this.lastInteractRep/10 != repData.rep/10 )
		{
			this.populateBuyingList(player, repData);
			this.needsInitilization = false;
			// this.lastInteractRep = repData.rep;
		}

		return net.minecraftforge.event.ForgeEventFactory.listTradeOffers(this, player, this.buyingList);
	}

	// public int lastInteractRep = -3000;

	private void populateBuyingList( EntityPlayer player, RepData repData )
	{
		if ( this.careerId != 0 && this.careerLevel != 0 )
		{
			this.careerLevel = MathHelper.clamp(1 + repData.rep / 500, 1, 8);
		}
		else
		{
	...
	public void readEntityFromNBT( NBTTagCompound compound )
	{
		...
		if ( compound.hasKey("ProfessionName") )
		{
			...
			if ( p == null ) p = net.minecraftforge.fml.common.registry.ForgeRegistries.VILLAGER_PROFESSIONS.getValue(new net.minecraft.util.ResourceLocation("minecraft:farmer"));
			this.setProfession(p);
		}

		this.unlockedGatedTrades = compound.getBoolean("GatedTrades");
	...
	public void writeEntityToNBT( NBTTagCompound compound )
	{
		...
		compound.setBoolean("GatedTrades", this.unlockedGatedTrades);
	...
	public void useRecipe( MerchantRecipe recipe ) // XXX
	{
		...
		if ( this.uiClick )
		{
			...
			if ( ToroQuestConfiguration.coinTradeSounds )
			{
				...
				case 0:
				{
					this.playSound(SoundHandler.TRADEC_0, this.getSoundVolume() * 0.9F, this.getSoundPitch());
					break;
				}
				case 1:
				{
					this.playSound(SoundHandler.TRADEC_1, this.getSoundVolume() * 0.9F, this.getSoundPitch());
					break;
				}
				case 2:
				{
					this.playSound(SoundHandler.TRADEC_2, this.getSoundVolume() * 0.9F, this.getSoundPitch());
					break;
				}
				}
			}
		...
		if ( recipe.getToolUses() >= 50000 )
		{
			this.playSound(SoundEvents.ENTITY_EXPERIENCE_ORB_PICKUP, this.getSoundVolume() * 0.9F, this.getSoundPitch());
			...
			{
				// this.getCustomer().closeScreen();
				
				RepData repData = this.getReputation(this.getCustomer());
				...
				{
					this.getRecipes(this.getCustomer(), repData);
				}
			}
	...
	public void setIsWillingToMate( boolean isWillingToMate )
	{
		if ( isWillingToMate )
		{
			this.isWillingToMate = this.getIsWillingToMate(true);
		}
		else
		{
	...
	{
		EntityRegistry.registerModEntity(new ResourceLocation(ToroQuest.MODID, NAME), EntityToroVillager.class, NAME, entityId, ToroQuest.INSTANCE, 80, 3, true, 0x000000, 0xe0d6b9);
	}
	...
	public boolean hasEnoughItems( int multiplier )
	{
		return true;
	}
	...
	public boolean isFarmItemInInventory()
	{
		for ( int i = 0; i < this.villagerInventory.getSizeInventory(); ++i )
		{
			...
			if ( !itemstack.isEmpty() && itemstack.getItem() instanceof net.minecraftforge.common.IPlantable )
			{
				return true;
			}
		}
